...
import { changeToppingStatus, getAllToppings, deleteTopping } from '~/services/toppingService';
import ToppingModalDetail from './toppingModalDetail';
import ToppingModalCreate from './toppingModalCreate';
...
const Topping = () => {
    ...
    const handleDeleteTopping = async (toppingId) => {
        try {
            const res = await deleteTopping(token, toppingId);
            if (res.status) {
                toast.success(res.message);
                setData(data.filter((topping) => topping.id !== toppingId));
            } else {
                ...
                Cell: ({ row }) => (
                    <Switch
                        ...
                        onChange={async (checked) => {
                            ...
                            try {
                                const res = await changeToppingStatus(token, row.original.id, {
                                    status: checked
                                });
                                const updatedData = data.map((topping) =>
                                    topping.id === row.original.id
                                        ? { ...topping, status: checked }
                                        : topping
                                );
                                toast.success(res.message);
                                setData(updatedData);
            ...
            },
            {
                ...
                Cell: ({ row }) => (
                    <div className='flex items-center w-full'>
                        <DeleteOutlined
                            className='text-3xl'
                            onClick={() => handleDeleteTopping(row.original.id)}
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const response = await getAllToppings(token, { page, limit });
    ...
    return (
        <div className='flex flex-col w-full'>
            ...
            <div className='py-5 bg-white shadow-sm rounded-xl h-fit'>
                ...
                <ToppingModalCreate isVisible={isModalCreate} onCancel={handleCancel} />
            </div>
        </div>
    );
};

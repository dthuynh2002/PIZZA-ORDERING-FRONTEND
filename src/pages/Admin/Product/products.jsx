...
import ProductModalDetail from '~/pages/Admin/Product/productModalDetail';
import {
    changeProductStatus,
    deleteProducts,
...
const Products = () => {
    ...
    const handleDeleteProduct = async (productId) => {
        try {
            const res = await deleteProducts(token, productId);
            if (res.status) {
                toast.success(res.message);
                setData(data.filter((size) => size.id !== productId));
            } else {
                ...
                Cell: ({ row }) => (
                    <Switch
                        ...
                        onChange={async (checked) => {
                            ...
                            try {
                                const res = await changeProductStatus(token, row.original.id, {
                                    status: checked
                                });
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const response = await getAllProductsPrivate(token, { page, limit });
    ...
    return (
        <div className='flex flex-col w-full'>
            ...
            <div className='py-5 bg-white shadow-sm rounded-xl h-fit'>
                ...
                <ProductModalCreate isVisible={isModalCreate} onCancel={handleCancel} />
            </div>
        </div>
    );
};

...
const ProtectedRoute = ({ children, roleRequired }) => {
    ...
    useEffect(() => {
        if (!loading) {
            if (!isAuthenticated) {
                navigate('/login');
            } else if (!roleRequired.includes(user.role_id)) {
                navigate('/');
            }
        }
    }, [isAuthenticated, user.role_id, roleRequired, navigate, loading]);
    ...
    return isAuthenticated && roleRequired.includes(user.role_id) ? children : null;
};
